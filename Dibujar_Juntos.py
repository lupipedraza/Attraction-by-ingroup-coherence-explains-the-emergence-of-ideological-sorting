...
def dibujar(Condicion,enElse,tipoP,k3,Nombre):
        ...
        for k1 in K:
            def ecuacion(t,x): #s<2/4
                ...
                dC,dI,dA,dT=cargar_ecuaciones(Condicion,enElse,tipoP,k3,C,I,A,T,k1)
                return([dC,dI,dA,dT])
        ...
        #Plotear simulacion
        lineObjects=ax.plot((1-df.T[0][1:]),df.T[range(1,5)][1:],marker='o',linewidth=1,alpha=0.7,markersize=4)

        
        #Plotear ecuaciones
        lineObjects_=ax.plot(1-K-k3,Finales,linewidth=3,linestyle='--',c='gray',alpha=0.7)
        ...
        return()
dibujar(Condicion,enElse,tipoP,k3,Nombre)
